...
def main():
    ...
    outer_path = "/".join([i for i in args.inputfile.split("/")[:-1]])
    #print(outer_path)
        ...
        for iteration, plane in enumerate(snd_planes):
            for hit in eval("event." + plane):
                if hit:
                    ...
                    if abs(pid) == 13:
                        filtered_muons_energies[iteration].append((weight, key_proj, x,y,z, px,py,pz))
        
        
    #np.savetxt("energy_filtered_keys_sco_0Point", np.array(filtered_muons_energies[0]))
    #np.savetxt("energy_filtered_keys_sco_1Point", np.array(filtered_muons_energies[1]))
    #np.savetxt("energy_filtered_keys_sco_2Point", np.array(filtered_muons_energies[2]))
    columns = ["weight", "key_proj", "x","y","z", "px","py","pz"]
    for k in range(3):
        #np.savetxt(f"energy_filtered_keys_sco_{k}Point_1file", np.array(filtered_muons_energies[k]))
        np.savetxt(f"energy_filtered_keys_sco_{k}Point_merged", np.array(filtered_muons_energies[k]))
        df_np = np.array(filtered_muons_energies[k], dtype=np.float64)
        df = r.RDF.MakeNumpyDataFrame({columns[i]: np.array(df_np[:,i], dtype=np.float64) for i in range(len(columns))})
        # ... or print the content
        #df.Display().Print()
        # ... or save the data as a ROOT file
        #df.Snapshot('tree', f'output_snd_planes_{k}_check_1file.root')
        df.Snapshot('tree', f'output_snd_planes_{k}_check_merged.root')
